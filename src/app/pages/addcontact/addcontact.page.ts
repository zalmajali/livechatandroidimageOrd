...
export class AddcontactPage implements OnInit {
  ...
  onFocus() {
    ...
    if (content) {
      // حفظ موقع الصفحة قبل فتح الكيبورد
      this.scrollPos = content.scrollTop;
    }
  }
  ...
  onBlur() {
    const content = document.querySelector('ion-content') as any;
    if (content) {
      // إعادة الصفحة لمكانها الأصلي بعد غلق الكيبورد
      content.scrollToPoint(0, this.scrollPos, 300);
    }
  }
