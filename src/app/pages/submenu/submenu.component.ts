...
export class SubmenuComponent  implements OnInit {
  ...
  @Input() userNameThisConv: string | any;
...
async initialiseTranslation(){
  ...
  this.translate.get('close_error_tow').subscribe((res: string) => {
   ...
   this.translate.get('close_error_Three').subscribe((res: string) => {
    this.close_error_Three = res;
  });
  ...
  functionCloseTo(type:any,number:any){
    ...
    this.chatService.chatClose(sendValues).then(async data=>{
      ...
      if(errorData == 1){
        ...
        this.displayResult(this.close_error_Three+this.userNameThisConv);
        this.modalController.dismiss({})
      }else{
